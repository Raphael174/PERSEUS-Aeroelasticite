...
@author: Raphaël, Eliot, Adrien, Marie-Bertille

version optimisée, 25% de gain de temps (17,66 s avant)
"""
...
def ClaCmaAilerons(d, a, b, s, m, n, supersonic=False):
    ...
    # ail = [ Xf, Cla ]
    ail = [e + (m * (a + 2 * b) / (3 * (a + b)) + 1 / 6 * (a + b - (a * b / (a + b)))),
           (4 * n * (s / d) ** 2) / (1 + (1 + ((2 * z) / (a + b)) ** 2) ** 1 / 2)]
...
def computeMoment(U_inf, Surf, chord, VecH, VecTeta, dCMa):
    ...
    Cm = (PI / 2) * b ** 2 * (
            a * b * VecH[2] / U_inf ** 2 - b ** 2 * (0.125 + a ** 2) * VecTeta[2] / U_inf ** 2 + b * (a - 0.5) *
            VecTeta[1] / U_inf)
...
NbIt = int(input("Number of steps to compute :\n>> "))
MaxTime = float(input("Duration of the simulation :\n>> "))
DT = MaxTime / NbIt
...
# ---------------------------------------------------------
# We create variables to monitor both
...
for CurrTime in range(0, NbIt):
    ...
    if CurrTime % (NbIt//10) == 0:
        print(f"{CurrTime // (NbIt//10)*10} %")

end = time.time()
print(f"Executed {NbIt} timesteps in {end - start} s\n")
...
DissRatio = DispH2[0]/DispTeta2[0]
...
print(f" | U_inf    = {U_inf} m/s  ->  {U_kmh} km/h")
print(f" | Dh/Da    = {round(DissRatio, 4)}")
print(f" | FreqH    = {FreqH}")
print(f" | FreqTeta = {FreqTeta}")
print(f" | FreqH/FreqTeta=  {round(FreqH / FreqTeta, 4)}")
