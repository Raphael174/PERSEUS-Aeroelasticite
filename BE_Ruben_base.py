...
def computeLift(q, U_inf, Surf, chord, VecH, VecTeta):
    ...
    FA += 2 * PI * computeCk(U_inf, chord) * rho * U_inf * b * (
            U_inf * VecTeta[0] + VecH[1] + (0.5 - a) * b * VecTeta[1])
    # ------------------
...
def computeMoment(q, U_inf, Surf, chord, VecH, VecTeta):
    ...
    a = S0 / (Mass * b)
    MA = rho * PI * b * b * (
            a * b * VecH[2] + U_inf * b * (a - 0.5) * VecTeta[1] - b * b * (0.125 + a * a) * VecTeta[2])
    MA += 2 * PI * computeCk(U_inf, chord) * rho * U_inf * b * b * (
            U_inf * VecTeta[0] + VecH[1] + (0.5 - a) * b * VecTeta[1])
    # ------------------
...
for CurrTime in range(0, NbIt):
    ...
    VecTime.append(Time)
        ...
        #print(f"{CurrTime // (NbIt // 100)} %")  PART OF LOOP ################
...
# what is FFTH and FFTeta? the discrete values representing the 
# # displacement and rotation trend?

# y_FTTH = fftpack.fft(DispH2)
# arrayH = np.array(DispH2)
# x_FreqH = fftpack.fftfreq(arrayH.size, d=DT)
# plt.plot(x_FreqH, np.abs(y_FTTH))
# plt.show()

# #%%

# i = FFTH.index(max(FFTH[0:(np.size(FFTH) - 1) // 2]))
# i2 = i = FFTH.index(max(FFTH[0:(np.size(FFTH) - 1)]))